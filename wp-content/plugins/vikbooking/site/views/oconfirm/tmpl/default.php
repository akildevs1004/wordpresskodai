...
<script type="text/javascript">
	...
	function validateVbSubmit() {
		...
		alert("Your Order is processsing. Please wait....");
...
<form id="finalConfirmation" action="<?php echo JRoute::rewrite('index.php?option=com_vikbooking' . (!empty($pitemid) ? '&Itemid=' . $pitemid : '')); ?>" name="vb" method="post" onsubmit="javascript: return validateVbSubmit();">
	<?php
			...
			?>
				...
				?>
					<div class="vbo-oconfirm-cfield-entry vbo-oconfirm-cfield-entry-checkbox">
						...
						<div class="vbo-oconfirm-cfield-input">
							<input type="checkbox" checked name="vbf<?php echo $cf['id']; ?>" id="vbf-inp<?php echo $cf['id']; ?>" value="<?php echo JText::translate('VBYES'); ?>" <?php echo !(bool)$cf['required'] && isset($customer_details['cfields']) && isset($customer_details['cfields'][$cf['id']]) ? 'checked="checked"' : ''; ?> />
						</div>
					</div>
			<?php
				}
	...
	<!-- <div id="loadercustom">
		<?php $preloader_logo = get_field('preloader_logo', 'options'); ?>

		<img src="<?php echo $preloader_logo ?>" alt="Image">
	</div> -->
	<!-- <?php echo $preloader_logo; ?> -->
	...
	<div class="vboconfirmbottom">
		<div>
			...
			<!-- <?php include('api_payment_gateway.php'); ?> -->
			<?php include('api_payment_gateway_razorpay.php'); ?>
		</div>
